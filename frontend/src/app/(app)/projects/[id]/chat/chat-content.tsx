...
export function ChatContent({ projectId }: { projectId: string }) {
  ...
  async function ensureSession(): Promise<string> {
    ...
    const s = await api.chat.createSession(projectId, "Ask PulseAI")
    setSessionId(s.id)
    return s.id
  }
  ...
  async function send(question: string) {
    ...
    try {
      ...
      await streamSSE(
        `/projects/${projectId}/chat/sessions/${sid}/messages`,
        { question },
        {
          onEvent: (e) => {
            if (e.type === "token") {
      ...
      <div ref={threadRef} className="flex-1 overflow-y-auto no-scrollbar py-6 space-y-5">
        {messages.length === 0 && (
          <div className="flex flex-col items-center text-center py-10">
            ...
            <p className="text-charcoal text-body-lg mt-5">Instant answers on status, blockers, and delivery risk.</p>
